...
public class FluidTankRenderer implements BlockEntityRenderer<FluidTankBlockEntity> {
    ...
    @Override
    public void render(FluidTankBlockEntity be, float partialTick, PoseStack poseStack,
        ...
        // Bottom rim
        drawCylinder(solid, mat, BODY_A0, 0, BODY_A1, RIM_H, BODY_B0, BODY_B1,
                bodySprite, 255, 255, 255, 255, packedLight, packedOverlay);
        // Top rim
        drawCylinder(solid, mat, BODY_A0, 1f - RIM_H, BODY_A1, 1f, BODY_B0, BODY_B1,
                bodySprite, 255, 255, 255, 255, packedLight, packedOverlay);
        ...
        if (be.isLocked()) {
            ...
            int fluidLight = fluid.getFluidType().getLightLevel() > 0
                    ? LightTexture.FULL_BRIGHT : packedLight;

            drawCylinder(translucent, mat, FLUID_A0, FLUID_FLOOR, FLUID_A1, fluidTop, FLUID_B0, FLUID_B1,
                    fluidSprite, fr, fg, fb, fa, fluidLight, packedOverlay);
        }

        // ---- Glass cylinder (translucent) — drawn after fluid so it overlays correctly ----
        drawCylinder(translucent, mat, GLASS_A0, GLASS_FLOOR, GLASS_A1, GLASS_CEIL, GLASS_B0, GLASS_B1,
                glassSprite, 255, 255, 255, 255, packedLight, packedOverlay);
     ...
     */
    private static void drawCylinder(VertexConsumer vc, Matrix4f mat,
            float xA0, float y0, float xA1, float y1, float zA0, float zA1,
            TextureAtlasSprite sp, int r, int g, int b, int a, int light, int overlay) {
    ...
    private static void drawBox(VertexConsumer vc, Matrix4f mat,
        ...
        // -Y (bottom)
        quad(vc, mat, r, g, b, a, light, overlay, u0, v0, u1, v1,
