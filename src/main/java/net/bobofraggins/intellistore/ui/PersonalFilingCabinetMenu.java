...
public class PersonalFilingCabinetMenu extends AbstractContainerMenu {
    ...
    public PersonalFilingCabinetMenu(int syncId, Inventory playerInv, int pfcSlot) {
        ...
        // Load folder contents from the PFC item's data component
        ItemStack pfcStack = playerInv.player.getInventory().getItem(pfcSlot);
    ...
    private void saveContentsToItem(Player player) {
        ...
        if (!(pfcStack.getItem() instanceof net.bobofraggins.intellistore.personalfilingcabinet.PersonalFilingCabinetItem)) return;

        List<ItemStack> folders = new ArrayList<>(FOLDER_SLOTS);
        for (int i = 0; i < FOLDER_SLOTS; i++) {
            folders.add(container.getItem(i).copy());
        }
        boolean voidExcess = isVoidExcess();
        pfcStack.set(Registration.PFC_CONTENTS.get(),
                new PersonalFilingCabinetContents(List.copyOf(folders), voidExcess));
    ...
    public static class Provider implements MenuProvider {
        ...
        public AbstractContainerMenu createMenu(int syncId, Inventory inv, Player player) {
            return new PersonalFilingCabinetMenu(syncId, inv, pfcSlot);
        }
    }
}
