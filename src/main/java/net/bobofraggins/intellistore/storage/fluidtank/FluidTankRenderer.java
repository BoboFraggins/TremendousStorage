 ...
 *
 * <p>The static shell is rendered by the block model ({@code models/block/fluid_tank.json}).
 * This BESR handles the translucent octagonal-prism fill and lazurite connector stubs.
 */
public class FluidTankRenderer extends AbstractTankRenderer<FluidTankBlockEntity> {

    public FluidTankRenderer(BlockEntityRendererProvider.Context ctx) {}

    @Override
    protected void renderFill(
            FluidTankBlockEntity be, Matrix4f mat, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
