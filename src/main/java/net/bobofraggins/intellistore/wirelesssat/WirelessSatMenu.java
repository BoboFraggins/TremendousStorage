 ...
 *
 * <p>Uses the same menu type as {@link StorageAccessTerminalMenu} so the client opens
 * {@link net.bobofraggins.intellistore.ui.StorageAccessTerminalScreen} automatically.
 ...
 */
public class WirelessSatMenu extends StorageAccessTerminalMenu {
     ...
     */
    public WirelessSatMenu(int id, Inventory inv, BlockPos niPos) {
    ...
    public boolean stillValid(Player player) {
        ...
        return playerHasLinkedWirelessSat(player, niPos);
    }

    private static boolean playerHasLinkedWirelessSat(Player player, BlockPos niPos) {
        // Check main inventory + off-hand
        for (ItemStack stack : player.getInventory().items) {
            if (isMatchingWirelessSat(stack, niPos)) return true;
        }
        if (isMatchingWirelessSat(player.getOffhandItem(), niPos)) return true;
            ...
            if (curiosInv != null) {
                for (var entry : curiosInv.getCurios().entrySet()) {
                    ...
                    for (int i = 0; i < handler.getSlots(); i++) {
                        if (isMatchingWirelessSat(handler.getStackInSlot(i), niPos)) return true;
                    }
                }
            }
    ...
    private static boolean isMatchingWirelessSat(ItemStack stack, BlockPos niPos) {
        if (!(stack.getItem() instanceof WirelessSatItem)) return false;
    ...
    public record Provider(BlockPos niPos) implements MenuProvider {
        ...
        public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
            return new WirelessSatMenu(id, inv, niPos);
        }
    }
}
