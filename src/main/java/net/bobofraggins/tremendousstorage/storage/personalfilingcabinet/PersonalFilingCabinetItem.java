...
import net.bobofraggins.tremendousstorage.shared.network.OpenPersonalFilingCabinetPacket;
...
public class PersonalFilingCabinetItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        player.openMenu(new PersonalFilingCabinetMenu.Provider(pfcSlot, null, null), buf -> buf.writeInt(pfcSlot));
        return InteractionResultHolder.success(stack);
    }
    ...
    public static void openPfcUi(ServerPlayer player, int slotType, int slotIndex, String slotId) {
        if (slotType == OpenPersonalFilingCabinetPacket.SLOT_INVENTORY) {
            ...
            player.openMenu(
                    new PersonalFilingCabinetMenu.Provider(slotIndex, null, null), buf -> buf.writeInt(slotIndex));
    ...
    private static void openFromCurios(ServerPlayer player, int slotIndex, String slotId) {
        try {
            ...
            player.openMenu(
                    new PersonalFilingCabinetMenu.Provider(slotIndex, slotId, entry), buf -> buf.writeInt(slotIndex));
