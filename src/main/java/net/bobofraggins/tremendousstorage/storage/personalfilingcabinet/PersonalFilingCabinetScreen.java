package net.bobofraggins.tremendousstorage.storage.personalfilingcabinet;

import net.bobofraggins.tremendousstorage.shared.network.SetPfcVoidExcessPacket;
import net.bobofraggins.tremendousstorage.shared.ui.AbstractFilingCabinetScreen;
import net.bobofraggins.tremendousstorage.shared.ui.ConfigDrawer;
import net.bobofraggins.tremendousstorage.shared.ui.VoidExcessPane;
import net.minecraft.network.chat.Component;
import net.minecraft.world.entity.player.Inventory;
import net.neoforged.neoforge.network.PacketDistributor;

/**
 * Screen for the Personal Filing Cabinet item.
 *
 * <p>Layout (176 × 270 px):
 * <ul>
 *   <li>Title centred in title bar; "≡" config button in top-left of title bar
 *   <li>Config drawer (slides left): void-excess toggle
 *   <li>8 folder slots (left col) + 8 extraction slots (right col), y=36..179
 *   <li>Player inventory 3×9 starting at y=186
 *   <li>Player hotbar at y=244
 * </ul>
 */
public class PersonalFilingCabinetScreen extends AbstractFilingCabinetScreen<PersonalFilingCabinetMenu> {

    private static final int BG_HEIGHT = 270;
    private static final int PLAYER_INV_Y = 186;

    public PersonalFilingCabinetScreen(PersonalFilingCabinetMenu menu, Inventory inv, Component title) {
        super(menu, inv, title, BG_HEIGHT, PLAYER_INV_Y, new ConfigDrawer(new VoidExcessPane(menu::isVoidExcess, () -> {
            boolean next = !menu.isVoidExcess();
            menu.setVoidExcess(next);
            PacketDistributor.sendToServer(new SetPfcVoidExcessPacket(menu.getPfcSlot(), next));
        })));
    }
}
