 ...
 * Grid extraction is handled by {@link net.bobofraggins.tremendousstorage.shared.network.LocalStorageInteractPacket}.
 *
 * <p>A single {@link ContainerData} slot syncs the current priority ordinal from server to client.
 */
public class TremendousChestMenu extends AbstractContainerMenu {
    ...
    public TremendousChestMenu(int id, Inventory inv, BlockPos pos, ContainerData data) {
        this(id, inv, pos, data, TremendousStorageClientConfig.ROWS_SCALE_4_PLUS_DEFAULT);
    }

    /** Client-side constructor. Reads the configured row count for the current GUI scale. */
    public TremendousChestMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(
                id,
                inv,
                buf.readBlockPos(),
     ...
     */
    private TremendousChestMenu(int id, Inventory inv, BlockPos pos, ContainerData data, int rows) {
    ...
    public void removed(Player player) {
        ...
        if (!player.level().isClientSide()) {
            if (player.level().getBlockEntity(pos) instanceof TremendousChestBlockEntity be) {
                be.stopOpen(player);
            }
        }
    }
